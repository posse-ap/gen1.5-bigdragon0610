                ...
                @foreach ($areas as $area)
                    <tr data-title_index="{{ $loop->index }}">
                        <form action="{{ route('edittitle.update', $area->id) }}" method="POST">
                            ...
                            @method('PUT')
                            <td>
                                <a href="editquestion/{{ $area->id }}"
                                    id="link_title_{{ $loop->index }}">{{ $area->name }}の難読地名クイズ</a>
                                <div id="edit_title_{{ $loop->index }}" class="invisible"><input type="text"
                                        name="name" value="{{ $area->name }}">の難読地名クイズ</div>
                            </td>
                            <td>
                                <div id="edit_title_button_{{ $loop->index }}" class="edit_title_button">編集</div>
                                <input type="submit" value="OK" id="edit_submit_button_{{ $loop->index }}"
                                    class="invisible">
                            </td>
                        </form>
                        <td>
